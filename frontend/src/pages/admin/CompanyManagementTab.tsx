...
import { deleteCompany, getCompaniesPagination } from '../../api/companyService'; // Importa tu servicio
...
export const CompanyManagementTab = () => {
  ...
  const [itemToEdit, setItemToEdit] = useState<Company | null>(null);

  const debouncedSearchTerm = useDebounce(searchTerm, 500);
  const fetchData = async () => {
      setLoading(true);
      const response = await getCompaniesPagination(currentPage, debouncedSearchTerm,  limit,sortBy, sortOrder);
  ...
  const columns = [
    ...
    const handleDelete = async (id: number) => {
      ...
      const handleOpenEditModal = (item: Company) => {
        setItemToEdit(item);
  ...
  return (
    <div className="space-y-4">
      <input type="text" placeholder="Buscar empresas..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full p-2 border rounded-md" />
